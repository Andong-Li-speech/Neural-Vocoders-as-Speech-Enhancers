...
class BSRNN(nn.Module):
    ...
    def forward(self, mel, inv_mel=None):
        ...
        for i in range(len(self.band_width)):
            cur_subband_spec = inv_amp[:, band_idx: band_idx + self.band_width[i]].transpose(-2, -1).contiguous()  # (B, T, fw)
        ...
        decode_mag, decode_phase = torch.exp(decode_resi_mag.transpose(-2, -1) + inv_amp).contiguous(), decode_phase.transpose(-2, -1)  # (B, F, T)
