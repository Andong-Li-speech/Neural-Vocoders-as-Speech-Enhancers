...
from dataset_joint_denoise_vocoder import Dataset, mel_spectrogram, amp_pha_specturm, get_dataset_filelist
...
def train(h):
    ...
    training_filelist, validation_filelist, training_noise_filelist, validation_noise_filelist = get_dataset_filelist(
        h.input_training_wav_list, h.input_validation_wav_list, h.raw_wavfile_path, h.input_noise_wav_list,
    )

    trainset = Dataset(
        training_filelist,
        training_noise_filelist,
        h.snr_range,
        h.segment_size,
        h.n_fft,
        h.num_mels,
    ...
    for epoch in tqdm(range(max(0, last_epoch), h.training_epochs)):
        start = time.time()
        ...
        for i, batch in enumerate(tqdm(train_loader)):
            ...
            x, logamp, pha, rea, imag, y, meloss = map(
                lambda x: x[0].to(device, non_blocking=True), batch
            )
            ...
            if steps % h.stdout_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    Mel_error = F.l1_loss(meloss, y_g_mel).item()  # modify the bug in the orig project
                    ...
                    for j, batch in enumerate(validation_loader):
                        ...
                        if j <= 10:
                            if steps == 0:
                                ...
                                sw.add_figure(
                                    "gt/y_spec_{}".format(j),
                                    plot_spectrogram(meloss.squeeze(0).cpu().numpy()),
                                    steps,
                                )
                            ...
                            sw.add_figure(
                                ...
                                plot_spectrogram(y_g_mel.squeeze(0).cpu().numpy()),
                                steps,
                            )
...
def main():
    ...
    parse.add_argument('--cfg_filename', type=str, required=True, default='./cfgs/bsrnn_joint_denoise_vocoder_config.json', help='Json for configurations.')
